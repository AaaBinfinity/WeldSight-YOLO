...
推理、任务编排、文件存储、MySQL 数据访问、PDF 生成以及 Kimi API 调用。该结构部署
...
| `inspection_store.py` | 管理 MySQL 表、搜索筛选、统计、人工复核和训练反馈 |
...
| 数据管理 | `/data.html` | 查看 MySQL 与本地存储、筛选记录、批量删除、导出和安全清理 |
...
统计接口按最近 7、30、90 天等时间窗口读取 MySQL 数据。看板中的“缺陷记录率”表示
...
数据管理页集中展示 MySQL 检测记录、人工复核完成率、本地文件数量和存储占用。记录表
支持按编号、文件名、项目、缺陷类别、来源和 AI 状态检索，勾选后可以生成只包含所选
记录的完整 ZIP 归档，或精确删除所选记录及其生成图片。

“导出全部 JSON”只包含结构化记录与报告元数据；“下载完整 ZIP”还会加入归档原图、
检测图和训练反馈文件。导出内容不会包含 MySQL 密码、Moonshot API Key 或服务器绝对
路径。孤立文件清理只删除数据库中已经不存在的记录目录和批次目录。清空全部检测数据
必须输入二次确认文字，操作会保留系统设置、模型参数与 API Key；后台仍有 AI 复核或
批量检测任务时，服务会拒绝执行清理或清空。
...
编辑根目录 `.env`，至少确认 MySQL 连接和 YOLO 权重路径。数据库需要提前创建，例如
`CREATE DATABASE WeldSight CHARACTER SET utf8mb4;`。需要 Kimi 复核时再填写新生成的
API Key：
...
MYSQL_PORT=3306
MYSQL_USER=root
MYSQL_PASSWORD=在这里填写_MySQL_密码
MYSQL_DATABASE=WeldSight
...
| `MYSQL_PASSWORD` | 空 | MySQL 密码，只保存在服务器 `.env` |
| `MYSQL_DATABASE` | `WeldSight` | MySQL 数据库名称 |
| `MYSQL_CONNECT_TIMEOUT` | `10` | MySQL 连接超时秒数 |
...
系统设置页修改的是 MySQL 中的运行时设置。数据库设置会覆盖相同项目的默认环境变量，
...
MySQL 使用 InnoDB 与 `utf8mb4` 字符集，应用启动时会自动创建三张业务表：
...
备份 MySQL 数据库与 `records` 目录，否则只有数据库而没有图片，历史详情和 PDF 将
无法完整恢复。迁移时必须保持数据库记录和文件目录的一致性。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| `GET` | `/api/data/overview` | 获取 MySQL 记录统计、本地文件统计和后台任务数量 |
| `GET` | `/api/data/export.json` | 导出全部或指定记录的脱敏 JSON |
| `GET` | `/api/data/export.zip?include_files=1` | 下载包含元数据、图片和反馈样本的 ZIP |
| `DELETE` | `/api/data/records` | 根据 JSON 请求体中的 `ids` 精确删除记录 |
| `POST` | `/api/data/cleanup` | 核对数据库并清理孤立目录 |
| `POST` | `/api/data/purge` | 清空检测记录、批次和本地业务文件，保留系统设置 |

导出指定记录时可以重复提供 `id` 查询参数，例如
`/api/data/export.zip?id=<记录一>&id=<记录二>&include_files=1`。删除、孤立清理与
全量清空均由前端提供确认流程；如果直接调用 API，应先备份数据并严格限制调用权限。
...
单条报告采用 A4 流式分页，并按阅读任务拆分为四个独立章节：首页为项目元数据、状态
指标与多色标注图；第二页为类别统计、综合结论和 YOLO 候选框明细；第三页为 AI 摘要、
确认发现、疑似误报、漏检和处置建议；第四页为人工结论、复核意见、修正记录和免责声明。
批量报告将批次概览与检测明细分成两页。各页使用更宽松的页边距、行距、单元格内边距和
章节留白，长文本与表格仍可自动换行、跨页，并在每页重复页眉、检测编号和页码。

PDF 使用 ReportLab 生成，并优先加载微软雅黑、黑体或 Noto CJK 字体。
...
先确认 MySQL 服务已启动、`WeldSight` 数据库已创建，并检查 `.env` 中的主机、端口、
用户名、密码和数据库名。修改 `.env` 后必须重启 Flask。若出现
`Access denied for user`，应检查账号密码及该账号对 `WeldSight` 的权限；若出现连接
超时，应检查 3306 端口、防火墙和 `MYSQL_HOST`。服务日志只记录安全的数据库地址，
不会输出密码。
...
数据库备份、模型权重和生成报告，并在修改检测结论逻辑时同步更新测试与文档。
